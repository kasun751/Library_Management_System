...
const AdultsSection = () => {
 ...
 const fetchCreations = async () => {
    try {
      const response = await axios.get('http://localhost:80/project_1/IdeaConer/Controller/AdultSectionController.php');
...
  };
  ...
  const handleAddImage = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:80/project_1/IdeaConer/Controller/AdultSectionController.php', formData, {
        headers: {
  ...
  const handleDeleteConfirm = async () => {
    if (selectedCreation) {
      try {
        const response = await axios.delete('http://localhost:80/project_1/IdeaConer/Controller/AdultSectionController.php', {
      ...
      <div className="row justify-content-center align-items-center">
        ...
        <ul className="creations-list">
          {creations.map((creation) => {
            const showDeleteButton = Number(creation.user_id) === Number(userId);
            return (
